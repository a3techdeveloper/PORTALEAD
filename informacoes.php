...
<div class="container-fluid">
    <div class="row"  style="margin-top: 100px;">    
        ...
        <div class="col-md-6" style="margin-top: 10px">
            <h5 class="h5 text-secondary">INFORMAÇÕES</h5>
            <?php
            include_once 'model/InformacaoDAO.class.php';
            $informacaoDAO = new InformacaoDAO();
            foreach ($informacaoDAO->listarTudo() as $info) {
                if($res->id_curso == $info->id_curso){
                ?>
                <div  style="border:1px solid #CCC; border-radius: 5px; padding: 14px; box-shadow:1px 1px 2px #000; margin-bottom: 5px; ">
                    <h5 class="h5 text-dark"><?= $info->titulo?></h5>
                    <p class="text-secondary lead"><?= $info->informacao ?></p>
                    <hr/>
                </div>
            <?php }} ?>
        </div>
